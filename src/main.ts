 ...
 ******************************************************************************************************************/
const username =
  Secrets.get("TODDLER_USERNAME") ||
  (process.env.TODDLER_USERNAME as string) ||
  "";
const password =
  Secrets.get("TODDLER_PASSWORD") ||
  (process.env.TODDLER_PASSWORD as string) ||
  "";
const worldId =
  Secrets.get("TODDLER_WORLD_ID") ||
  (process.env.TODDLER_WORLD_ID as string) ||
  "";
const natsUri = process.env.TODDLER_NATS_URI || "nats://demo.nats.io";
let nc: Client;
...
let liveCheck = new LivenessCheck("liveCheck", livePromise);
healthcheck.registerLivenessCheck(liveCheck);

let endpoint = express();
endpoint.use("/live", LivenessEndpoint(healthcheck));
endpoint.use("/ready", ReadinessEndpoint(healthcheck));
endpoint.use("/", HealthEndpoint(healthcheck));
endpoint.listen(process.env.PORT || 8080, () => {});
...
function setupSubscriptionForStatus(nc: Client): Promise<void> {
  return new Promise<void>(resolve => {
    nc.subscribe("authenticator.running", (error, msg) => {
      Log.service().debug("Received ready check");
      if (msg.reply) {
        Log.service().debug("Ready check sent");
        nc.publish(msg.reply as string, {
          running: isServiceLive
        });
      }
    }).then(() => {
      resolve();
...
function setupSubscriptionsToProcessRequests(
  nc: Client,
  requestTypes: string[]
    ...
    requestTypes.map(type => {
      ...
      return nc.subscribe(">", (error, msg) => {
        if (error) {
          ...
          if (msg.subject != "authenticator.ready") {
...
function enableNotifications() {
  ...
  authenticator.on(
    AuthenticatorEvent.unattachedMessageReceived,
    (msg: ISocketMessage) => {
      nc.publish(msg.type.toLowerCase().replace("/", "."), msg.data);
    }
  );
}
...
function setupSubscriptionToGame(): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    nc.subscribe("authenticator.to.game", (err, msg) => {
      if (err) {
