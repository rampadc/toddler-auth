...
if (mqUrl.trim().length == 0) {
  Log.service().error('MQ URL is not provided.');
    process.exit(1);
}
...
function checkReadiness() {
    if (!authenticator.isAuthenticated()) {
      Log.service().error('Socket client is not authenticated');
        return;
    } else {
      Log.service().debug('Socket client is authenticated');
    }

    if (!authenticator.isMQSetupCompleted()) {
      Log.service().error('MQ setup not completed');
        authenticator.connectMQ();
        return;
    } else {
      Log.service().debug('MQ setup completed');
    }

  Log.service().info('Initialization completed');
